...
RSpec.describe SmartName::Parts do
  ...
  describe '#replace_part' do
    ...
    it 'replaces two name parts' do
      expect('a+b+c'  .to_name.replace('a+b', 'x').to_s).to eq('x+c')
      expect('a+b+c+d'.to_name.replace('a+b', 'e+f').to_s).to eq('e+f+c+d')
    end
    it "doesn't replace two part tag" do
      expect('a+b+c'.to_name.replace('b+c', 'x').to_s).to eq('a+b+c')
    end
  end
end
